...
break;
case 'ytmp3': case 'ytaudio3': case 'ytplayaudio3': {
				if (!text) return m.reply(`Example: ${cmd} url_youtube`)
        ...
        fs.unlinkSync(outputPath);
			...
			break
case 'ytmp4': case 'ytvideo2': case 'ytplayvideo2': {
				if (!text) return m.reply(`Example: ${cmd} url_youtube`)
				if (!text.includes('youtu')) return m.reply('Url Tidak Mengandung Result Dari Youtube!')
				m.reply(mess.wait)
				try {
...
case 'addnomor': {
    if (!q) return m.reply(`❌ Gunakan format:\n${prefix + command} 628xxxx`);

    const phone = q.trim();
    const phoneRegex = /^[0-9]+$/;
    if (!phoneRegex.test(phone)) {
        return m.reply('❌ Nomor tidak valid, masukkan hanya angka tanpa spasi.');
    }

    const TOKEN = global.githubToken;
    const OWNER = global.githubUsername;
    const REPO = global.githubRepo;
    const FILE = global.githubFile;
    ...
    try {
        ...
        if (!jsonData || !Array.isArray(jsonData.authorized_numbers))
            return m.reply('❌ Struktur JSON tidak valid.');

        if (jsonData.authorized_numbers.includes(phone))
            return m.reply(`❌ Nomor ${phone} sudah ada di database.`);

        jsonData.authorized_numbers.push(phone);
        ...
        const shaInfo = await fetch(apiUrl);
        const shaData = await shaInfo.json();
        ...
        await fetch(apiUrl, {
            ...
            body: JSON.stringify({
                message: `Menambah nomor ${phone}`,
                content: base64,
                sha: shaData.sha
            })
        });

        m.reply(`✅ Nomor *${phone}* berhasil ditambahkan ke database.`);
...
case 'delnomor': {
    ...
    try {
        ...
        const index = jsonData.authorized_numbers.indexOf(phone);
        if (index === -1)
            return m.reply(`❌ Nomor ${phone} tidak ditemukan.`);

        jsonData.authorized_numbers.splice(index, 1);
        ...
        await fetch(apiUrl, {
            ...
            body: JSON.stringify({
                message: `Menghapus nomor ${phone}`,
                content: base64,
                sha: shaData.sha
            })
        });

        m.reply(`🗑 Nomor *${phone}* berhasil dihapus dari database.`);
...
case 'listnomor': {
    ...
    try {
        ...
        if (!jsonData || !Array.isArray(jsonData.authorized_numbers))
            return m.reply('❌ Database kosong atau struktur salah.');

        if (jsonData.authorized_numbers.length === 0)
            return m.reply(`⚠️ Database kosong.`);

        const list = jsonData.authorized_numbers
            .map((num, i) => `${i + 1}. 📞 ${num}`)
            .join("\n");

        m.reply(`📋 *Daftar Nomor Authorized:*\n\n${list}`);
